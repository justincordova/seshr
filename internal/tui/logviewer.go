...
// LogViewer displays the last N lines of ~/.agentlens/debug.log in a viewport.
...
func (lv LogViewer) View() string {
	title := textStyle.Bold(true).Render("Debug Log") +
		"  " + dimStyle.Render("~/.agentlens/debug.log")
...
func (lv LogViewer) loadLog() string {
	...
	path := filepath.Join(home, ".agentlens", "debug.log")
